...
async function main() {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const t40 = await tx.transaction.findFirstOrThrow({
      where: { bankAccount: { companyId: CO }, amount: 40000, date: new Date('2026-07-06T12:00:00Z'), description: { contains: 'arafat' } },
      select: { id: true, categoryId: true },
    })
    const t50 = await tx.transaction.findFirstOrThrow({
      where: { bankAccount: { companyId: CO }, amount: 50000, date: new Date('2026-08-04T12:00:00Z'), description: { contains: 'rafat' } },
      select: { id: true, categoryId: true },
    })
    ...
    await tx.loanInstallment.update({ where: { id: insts[0].id }, data: { status: 'PAID', amortization: 40000, interest: 0, correcao: 0, paidTotal: 40000, paidInterest: 0, paidCorrection: 0, paidDate: new Date('2026-07-06T12:00:00Z'), reconciledTransactionId: t40.id } })
    // 3) parcela #2 = devolução 50.000 (04/08), vinculada à tx 50k
    await tx.loanInstallment.update({ where: { id: insts[1].id }, data: { status: 'PAID', amortization: 50000, interest: 0, correcao: 0, paidTotal: 50000, paidInterest: 0, paidCorrection: 0, paidDate: new Date('2026-08-04T12:00:00Z'), reconciledTransactionId: t50.id } })
