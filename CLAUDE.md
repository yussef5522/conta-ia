...
- **Paridade V1/V2 do import OFX — gaps MEDIDOS (Etapa 2, 06/08)** — medição real em Postgres scratch (`scripts/measure-v1v2-parity.ts`, guarda em `__tests__/reconciliation/v1v2-parity-gaps.test.ts`). **COBERTO (não é gap):** re-import do mesmo OFX → 0 novas (reconcileStatement dedup por stableKey, NÃO depende do ImportedIdentity ledger); categoryOverrides, ledgerBal/saldo, detecção-transfer (via banner). **GAPS REAIS pra Etapa 3:** (a) **ALTO — SKIP decisions**: `runImportV2` nem aceita `decisions` no input; linha desmarcada no preview ENTRA mesmo assim (preview≠confirm); (b) MÉDIO — não auto-classifica por regra/keyword no import (tudo nasce PENDING; overrides manuais funcionam); (c) BAIXO — não popula `fitidKey`/`contentHash` nem seed `ImportedIdentity` (dedup OK mesmo assim; efeito só em placeholder-reconcile de TRANSFER e audit). **Não corrigidos — Etapa 3 trata um por vez.**
