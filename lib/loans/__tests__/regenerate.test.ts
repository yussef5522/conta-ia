...
const loan: RegenLoan = {
  ...
  financedAmount: null, firstDueDate: new Date('2026-01-15'),
}
...
describe('regenerateSchedule', () => {
  ...
  it('parcela baixa demais (parcela×prazo < saldo) → não fecha, NÃO grava', () => {
    const r = regenerateSchedule(loan, seed(), { system: 'PRICE', parcela: 2700, rateMonthly: 0.017, isPostFixed: true })
  ...
  it('pré-fixado com taxa 0 seria pego pelo guard (não silencia)', () => {
    const r = regenerateSchedule(loan, seed(), { system: 'PRICE', parcela: 3800, rateMonthly: 0, isPostFixed: false })
