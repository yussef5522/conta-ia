...
4. \`detectedBank\` = "C6 Bank"`,GENERIC:r};class o extends Error{code;constructor(e,t){super(e),this.code=t,this.name="PdfExtractError"}}e.s(["PdfExtractError",0,o],21513);let s=["MOBILE_PHOTO"];function i(e){return{...e,detectedCardLast4:e.detectedCardLast4?e.detectedCardLast4.slice(-4):null}}async function d(e,t={}){var a;let r,i,c;if(0===e.pdfBytes.length)throw new o("PDF vazio","PDF_INVALID");if(e.pdfBytes.length>5242880)throw new o(`PDF muito grande (${(e.pdfBytes.length/1024/1024).toFixed(1)} MB; m\xe1ximo 5 MB)`,"PDF_TOO_LARGE");let u=new TextDecoder("utf-8",{fatal:!1}).decode(e.pdfBytes.slice(0,Math.min(2048,e.pdfBytes.length)));if(!u.startsWith("%PDF-"))throw new o("Arquivo não parece ser um PDF válido (sem header %PDF-).","PDF_INVALID");if(/\/Encrypt\b/.test(u))throw new o("PDF está com senha/criptografia. Remova a proteção e tente de novo.","PDF_ENCRYPTED");let p=t.apiKey??process.env.ANTHROPIC_API_KEY;if(!p)throw new o("ANTHROPIC_API_KEY não configurada no servidor.","CLAUDE_API_ERROR");let x=t.modelOverride??process.env.AI_CLAUDE_VISION_MODEL??"claude-sonnet-4-6",m=t.fetch??globalThis.fetch,f=t.timeoutMs??3e4,h=e.bankHint??((c=e.fileName.toLowerCase()).includes("nubank")||c.includes("nu_pagamentos")?"NUBANK":c.includes("itau")||c.includes("itaú")?"ITAU":c.includes("bradesco")?"BRADESCO":c.includes("inter")?"INTER":c.includes("c6")?"C6":"GENERIC"),E=n[h],A={model:x,max_tokens:4e3,messages:[{role:"user",content:[{type:"document",source:{type:"base64",media_type:"application/pdf",data:Buffer.from(e.pdfBytes).toString("base64")}},{type:"text",text:E}]}]},C=new AbortController,T=setTimeout(()=>C.abort(),f),I=Date.now();try{r=await m("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"x-api-key":p,"anthropic-version":"2023-06-01","content-type":"application/json"},body:JSON.stringify(A),signal:C.signal})}catch(t){if(clearTimeout(T),console.error("[pdf-extract] network error",{durationMs:Date.now()-I,bank:h,pdfSize:e.pdfBytes.length}),t instanceof Error&&"AbortError"===t.name)throw new o("Timeout na API Claude Vision","CLAUDE_TIMEOUT");throw new o("Falha de rede ao chamar Claude Vision","CLAUDE_API_ERROR")}if(clearTimeout(T),429===r.status)throw new o("Rate limit Claude","CLAUDE_RATE_LIMITED");if(!r.ok)throw console.error("[pdf-extract] api error",{status:r.status,bank:h,durationMs:Date.now()-I,pdfSize:e.pdfBytes.length}),new o(`Erro Claude API ${r.status}`,"CLAUDE_API_ERROR");let R=await r.json(),g=R.content?.[0]?.text??"",S=R.usage?.input_tokens??0,D=R.usage?.output_tokens??0,O=Math.round(100*(S/1e6*Number(process.env.AI_CLAUDE_VISION_PRICE_INPUT_USD??3)+D/1e6*Number(process.env.AI_CLAUDE_VISION_PRICE_OUTPUT_USD??15))*100);try{let e=function(e){let t=e.trim();if(t.startsWith("{"))return t;let a=t.indexOf("{");if(a<0)throw Error("no JSON found");let r=0;for(let e=a;e<t.length;e++){let n=t[e];if("{"===n)r++;else if("}"===n&&0==--r)return t.slice(a,e+1)}throw Error("unbalanced JSON")}(g);i=JSON.parse(e)}catch{throw console.error("[pdf-extract] invalid json",{bank:h,textPreview:g.slice(0,200)}),new o("Claude retornou JSON inválido","CLAUDE_INVALID_JSON")}let v=Array.isArray(i.transactions)?i.transactions.map((e,t)=>{var a,r;let n,o,s,i;return a=e,r=t,n=String(a.memo??"").trim().slice(0,200),o=String(a.date??"").slice(0,10),s=Math.abs(l(a.amount)??0),i="CREDIT"===a.type?"CREDIT":"DEBIT",{fitid:a.fitid&&"string"==typeof a.fitid?a.fitid:`PDF-${o||"nodate"}-${n.replace(/[^a-zA-Z0-9]+/g,"-").slice(0,20).toLowerCase()||`tx${r}`}-${s.toFixed(2)}`,date:o,amount:s,type:i,memo:n,lineConfidence:l(a.lineConfidence)??.8,isInternational:!0===a.isInternational,originalCurrency:"string"==typeof a.originalCurrency?a.originalCurrency:void 0,originalAmount:l(a.originalAmount)??void 0}}):[],P="DIGITAL"===(a=i.scanQuality)||"SCANNED_HIGH"===a||"SCANNED_LOW"===a||"MOBILE_PHOTO"===a||"UNKNOWN"===a?i.scanQuality:"UNKNOWN",N={detectedBank:i.detectedBank??null,scanQuality:P,closingDate:i.closingDate??null,dueDate:i.dueDate??null,declaredTotal:l(i.declaredTotal),extractedSum:l(i.extractedSum)??v.reduce((e,t)=>e+("DEBIT"===t.type?t.amount:-t.amount),0),declaredTxCount:l(i.declaredTxCount),confidence:l(i.confidence)??.7,detectedCardLast4:i.detectedCardLast4??null,transactions:v,warnings:Array.isArray(i.warnings)?i.warnings.map(String):[],modelVersion:R.model??x,inputTokens:S,outputTokens:D,costCentsUsdX100:O},{result:w,shouldReject:y,rejectReason:b}=function(e){let t=[...e.warnings??[]],a=[];if(s.includes(e.scanQuality))return{result:e,shouldReject:!0,rejectReason:"Detectamos que isso é uma foto de celular. Por favor, envie o PDF DIGITAL da fatura (baixado do app ou site do banco), não uma foto."};if(null!=e.declaredTotal&&null!=e.extractedSum&&isFinite(e.declaredTotal)&&isFinite(e.extractedSum)){let r=Math.abs(e.declaredTotal-e.extractedSum),n=e.declaredTotal>0?r/e.declaredTotal:0;r>.5&&(t.push(`⚠️ Soma das transa\xe7\xf5es (R$ ${e.extractedSum.toFixed(2)}) ≠ total da fatura (R$ ${e.declaredTotal.toFixed(2)}). Diferen\xe7a R$ ${r.toFixed(2)}.`),n<.02?a.push(.85):n<.1?a.push(.6):a.push(.4))}else null==e.declaredTotal&&(t.push("Não detectamos o total da fatura no PDF — não foi possível validar a soma."),a.push(.85));null!=e.declaredTxCount&&Math.abs(e.declaredTxCount-e.transactions.length)>1&&(t.push(`⚠️ PDF declara ${e.declaredTxCount} transa\xe7\xf5es, extra\xedmos ${e.transactions.length}.`),a.push(.7));let r=e.transactions.length,n=1;switch(r>0&&(n=e.transactions.reduce((e,t)=>e+(t.lineConfidence??1),0)/r),n<.85&&a.push(n),e.scanQuality){case"SCANNED_LOW":a.push(.55),t.push("⚠️ Qualidade do scan é BAIXA. Revise CADA linha antes de confirmar.");break;case"SCANNED_HIGH":a.push(.85);break;case"UNKNOWN":a.push(.7)}let o=e.confidence??.85;for(let e of a)o*=e;return o=Math.max(0,Math.min(1,o)),{result:{...e,confidence:o,warnings:t},shouldReject:!1}}(N);if(y)throw new o(b??"PDF rejeitado","IS_PHOTO_REJECTED");return console.log("[pdf-extract] success",{bank:h,durationMs:Date.now()-I,pdfSize:e.pdfBytes.length,inputTokens:S,outputTokens:D,costCentsUsdX100:O,modelVersion:N.modelVersion,detectedBank:N.detectedBank,txCount:N.transactions.length,confidence:w.confidence,scanQuality:N.scanQuality}),w}function l(e){if("number"==typeof e&&Number.isFinite(e))return e;if("string"==typeof e){let t=Number.parseFloat(e.replace(",","."));if(Number.isFinite(t))return t}return null}async function c(e){let a=await t.prisma.personalPdfExtractCache.findUnique({where:{pdfSha256:e}});if(!a)return null;if(a.expiresAt<new Date)return t.prisma.personalPdfExtractCache.delete({where:{id:a.id}}).catch(()=>{}),null;t.prisma.personalPdfExtractCache.update({where:{id:a.id},data:{hitCount:{increment:1}}}).catch(()=>{});try{return JSON.parse(a.resultJson)}catch{return null}}async function u(e){let a=i(e.result),r=new Date(Date.now()+6048e5);await t.prisma.personalPdfExtractCache.upsert({where:{pdfSha256:e.pdfSha256},create:{pdfSha256:e.pdfSha256,modelVersion:a.modelVersion,resultJson:JSON.stringify(a),inputTokens:a.inputTokens,outputTokens:a.outputTokens,costCentsUsdX100:a.costCentsUsdX100,ownerUserId:e.ownerUserId,expiresAt:r},update:{expiresAt:r,hitCount:{increment:1}}})}async function p(e,a){let r=await t.prisma.personalPdfExtractCache.findUnique({where:{pdfSha256:e}});return!!r&&r.ownerUserId===a&&(await t.prisma.personalPdfExtractCache.delete({where:{id:r.id}}),!0)}async function x(e){return t.prisma.personalPdfExtractCache.findMany({where:{ownerUserId:e},select:{id:!0,pdfSha256:!0,modelVersion:!0,inputTokens:!0,outputTokens:!0,costCentsUsdX100:!0,hitCount:!0,cachedAt:!0,expiresAt:!0},orderBy:{cachedAt:"desc"}})}e.s(["extractFromPdf",0,d,"sanitizeForCache",0,i,"sha256Pdf",0,function(e){return(0,a.createHash)("sha256").update(e).digest("hex")}],373342),e.s(["deleteCachedExtraction",0,p,"getCachedExtraction",0,c,"listOwnerCaches",0,x,"saveCachedExtraction",0,u],35155)},219101,e=>{"use strict";var t=e.i(747909),a=e.i(174017),r=e.i(996250),n=e.i(759756),o=e.i(561916),s=e.i(174677),i=e.i(869741),d=e.i(316795),l=e.i(487718),c=e.i(995169),u=e.i(47587),p=e.i(666012),x=e.i(570101),m=e.i(520536),f=e.i(10372),h=e.i(193695);e.i(820232);var E=e.i(600220),A=e.i(89171),C=e.i(368105),T=e.i(35155);async function I(e){let t=await (0,C.getAuthUser)(e);if(!t)return A.NextResponse.json({erro:"Não autenticado"},{status:401});let a=await (0,T.listOwnerCaches)(t.sub);return A.NextResponse.json({caches:a})}e.s(["GET",0,I],128129);var R=e.i(128129);let g=new t.AppRouteRouteModule({definition:{kind:a.RouteKind.APP_ROUTE,page:"/api/auth/me/pdf-cache/route",pathname:"/api/auth/me/pdf-cache",filename:"route",bundlePath:""},distDir:"/tmp/nb",relativeProjectDir:"",resolvedPagePath:"[project]/app/api/auth/me/pdf-cache/route.ts",nextConfigOutput:"",userland:R,...{}}),{workAsyncStorage:S,workUnitAsyncStorage:D,serverHooks:O}=g;async function v(e,t,r){r.requestMeta&&(0,n.setRequestMeta)(e,r.requestMeta),g.isDev&&(0,n.addRequestMeta)(e,"devRequestTimingInternalsEnd",process.hrtime.bigint());let A="/api/auth/me/pdf-cache/route";A=A.replace(/\/index$/,"")||"/";let C=await g.prepare(e,t,{srcPage:A,multiZoneDraftMode:!1});if(!C)return t.statusCode=400,t.end("Bad Request"),null==r.waitUntil||r.waitUntil.call(r,Promise.resolve()),null;let{buildId:T,params:I,nextConfig:R,parsedUrl:S,isDraftMode:D,prerenderManifest:O,routerServerContext:v,isOnDemandRevalidate:P,revalidateOnlyGenerated:N,resolvedPathname:w,clientReferenceManifest:y,serverActionsManifest:b}=C,_=(0,i.normalizeAppPath)(A),U=!!(O.dynamicRoutes[_]||O.routes[w]),M=async()=>((null==v?void 0:v.render404)?await v.render404(e,t,S,!1):t.end("This page could not be found"),null);if(U&&!D){let e=!!O.routes[w],t=O.dynamicRoutes[_];if(t&&!1===t.fallback&&!e){if(R.adapterPath)return await M();throw new h.NoFallbackError}}let B=null;!U||g.isDev||D||(B="/index"===(B=w)?"/":B);let L=!0===g.isDev||!U,k=U&&!L;b&&y&&(0,s.setManifestsSingleton)({page:A,clientReferenceManifest:y,serverActionsManifest:b});let F=e.method||"GET",q=(0,o.getTracer)(),H=q.getActiveScopeSpan(),j=!!(null==v?void 0:v.isWrappedByNextServer),$=!!(0,n.getRequestMeta)(e,"minimalMode"),G=(0,n.getRequestMeta)(e,"incrementalCache")||await g.getIncrementalCache(e,R,O,$);null==G||G.resetRequestCache(),globalThis.__incrementalCache=G;let V={params:I,previewProps:O.preview,renderOpts:{experimental:{authInterrupts:!!R.experimental.authInterrupts},cacheComponents:!!R.cacheComponents,supportsDynamicResponse:L,incrementalCache:G,cacheLifeProfiles:R.cacheLife,waitUntil:r.waitUntil,onClose:e=>{t.on("close",e)},onAfterTaskError:void 0,onInstrumentationRequestError:(t,a,r,n)=>g.onRequestError(e,t,r,n,v)},sharedContext:{buildId:T}},Y=new d.NodeNextRequest(e),K=new d.NodeNextResponse(t),X=l.NextRequestAdapter.fromNodeNextRequest(Y,(0,l.signalFromNodeResponse)(t));try{let n,s=async e=>g.handle(X,V).finally(()=>{if(!e)return;e.setAttributes({"http.status_code":t.statusCode,"next.rsc":!1});let a=q.getRootSpanAttributes();if(!a)return;if(a.get("next.span_type")!==c.BaseServerSpan.handleRequest)return void console.warn(`Unexpected root span type '${a.get("next.span_type")}'. Please report this Next.js issue https://github.com/vercel/next.js`);let r=a.get("next.route");if(r){let t=`${F} ${r}`;e.setAttributes({"next.route":r,"http.route":r,"next.span_name":t}),e.updateName(t),n&&n!==e&&(n.setAttribute("http.route",r),n.updateName(t))}else e.updateName(`${F} ${A}`)}),i=async n=>{var o,i;let d=async({previousCacheEntry:a})=>{try{if(!$&&P&&N&&!a)return t.statusCode=404,t.setHeader("x-nextjs-cache","REVALIDATED"),t.end("This page could not be found"),null;let o=await s(n);e.fetchMetrics=V.renderOpts.fetchMetrics;let i=V.renderOpts.pendingWaitUntil;i&&r.waitUntil&&(r.waitUntil(i),i=void 0);let d=V.renderOpts.collectedTags;if(!U)return await (0,p.sendResponse)(Y,K,o,V.renderOpts.pendingWaitUntil),null;{let e=await o.blob(),t=(0,x.toNodeOutgoingHttpHeaders)(o.headers);d&&(t[f.NEXT_CACHE_TAGS_HEADER]=d),!t["content-type"]&&e.type&&(t["content-type"]=e.type);let a=void 0!==V.renderOpts.collectedRevalidate&&!(V.renderOpts.collectedRevalidate>=f.INFINITE_CACHE)&&V.renderOpts.collectedRevalidate,r=void 0===V.renderOpts.collectedExpire||V.renderOpts.collectedExpire>=f.INFINITE_CACHE?void 0:V.renderOpts.collectedExpire;return{value:{kind:E.CachedRouteKind.APP_ROUTE,status:o.status,body:Buffer.from(await e.arrayBuffer()),headers:t},cacheControl:{revalidate:a,expire:r}}}}catch(t){throw(null==a?void 0:a.isStale)&&await g.onRequestError(e,t,{routerKind:"App Router",routePath:A,routeType:"route",revalidateReason:(0,u.getRevalidateReason)({isStaticGeneration:k,isOnDemandRevalidate:P})},!1,v),t}},l=await g.handleResponse({req:e,nextConfig:R,cacheKey:B,routeKind:a.RouteKind.APP_ROUTE,isFallback:!1,prerenderManifest:O,isRoutePPREnabled:!1,isOnDemandRevalidate:P,revalidateOnlyGenerated:N,responseGenerator:d,waitUntil:r.waitUntil,isMinimalMode:$});if(!U)return null;if((null==l||null==(o=l.value)?void 0:o.kind)!==E.CachedRouteKind.APP_ROUTE)throw Object.defineProperty(Error(`Invariant: app-route received invalid cache entry ${null==l||null==(i=l.value)?void 0:i.kind}`),"__NEXT_ERROR_CODE",{value:"E701",enumerable:!1,configurable:!0});$||t.setHeader("x-nextjs-cache",P?"REVALIDATED":l.isMiss?"MISS":l.isStale?"STALE":"HIT"),D&&t.setHeader("Cache-Control","private, no-cache, no-store, max-age=0, must-revalidate");let c=(0,x.fromNodeOutgoingHttpHeaders)(l.value.headers);return $&&U||c.delete(f.NEXT_CACHE_TAGS_HEADER),!l.cacheControl||t.getHeader("Cache-Control")||c.get("Cache-Control")||c.set("Cache-Control",(0,m.getCacheControlHeader)(l.cacheControl)),await (0,p.sendResponse)(Y,K,new Response(l.value.body,{headers:c,status:l.value.status||200})),null};j&&H?await i(H):(n=q.getActiveScopeSpan(),await q.withPropagatedContext(e.headers,()=>q.trace(c.BaseServerSpan.handleRequest,{spanName:`${F} ${A}`,kind:o.SpanKind.SERVER,attributes:{"http.method":F,"http.target":e.url}},i),void 0,!j))}catch(t){if(t instanceof h.NoFallbackError||await g.onRequestError(e,t,{routerKind:"App Router",routePath:_,routeType:"route",revalidateReason:(0,u.getRevalidateReason)({isStaticGeneration:k,isOnDemandRevalidate:P})},!1,v),U)throw t;return await (0,p.sendResponse)(Y,K,new Response(null,{status:500})),null}}e.s(["handler",0,v,"patchFetch",0,function(){return(0,r.patchFetch)({workAsyncStorage:S,workUnitAsyncStorage:D})},"routeModule",0,g,"serverHooks",0,O,"workAsyncStorage",0,S,"workUnitAsyncStorage",0,D],219101)}];

//# sourceMappingURL=%5Broot-of-the-server%5D__12gbxnn._.js.map