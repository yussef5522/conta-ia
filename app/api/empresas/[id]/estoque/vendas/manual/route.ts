...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  const { data, entradas, confirmar, confirmouSanidade } = parsed.data
  try {
    if (confirmar) return NextResponse.json({ ok: true, recibo: await confirmarLancamentoManual(companyId, data, entradas, user.sub, prisma, confirmouSanidade ?? false) })
    return NextResponse.json({ plano: await previewLancamentoManual(companyId, data, entradas, user.sub, prisma) })
  } catch (e) {
    // ⭐ 409 = pergunta (N× a média), não erro — a tela mostra os números e o dono decide.
