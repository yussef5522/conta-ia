...
export default async function PermissoesPage({ params }: PageProps) {
  ...
  const hasView = permissionMatches(permKeys, 'role.view')
  const hasCreate = permissionMatches(permKeys, 'role.create')
  const hasUpdate = permissionMatches(permKeys, 'role.update')
  const hasDelete = permissionMatches(permKeys, 'role.delete')
      ...
      <div className="container max-w-4xl py-8">
        <div className="rounded-lg border border-orange-200 bg-orange-50 p-6 dark:border-orange-900/50 dark:bg-orange-950/30">
          ...
          <p className="mt-2 text-sm text-orange-800 dark:text-orange-200">
            Você não tem permissão pra visualizar roles desta empresa.
          </p>
        </div>
      </div>
    )
