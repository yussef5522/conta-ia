...
// ESTOQUE FASE 3 — import de vendas do Suitable. Fluxo dos líderes (Apicbase/MarketMan):
// mapear = CADASTRO PERMANENTE (uma vez, vale sempre); não-mapeado = exceção que NÃO trava
// (vai pra pendentes); processar = botão claro → preview → confirma → recibo do dia.
// Diário de 30s: sobe o arquivo, conhecidos resolvem sozinhos, só produto novo pergunta.

import { useMemo, useRef, useState, use } from 'react'
import { Card, CardContent } from '@/components/ui/card'
import { ShoppingCart, Loader2, Upload, Check, Pencil, Search, Play, X, Receipt, AlertTriangle, Package } from 'lucide-react'
...
interface Plano { produtos: { nome: string; quantidade: number; alvoNome: string; baixa: { nome: string; qtd: number; custoMedio: number | null }[] }[]; pendentes: { nome: string; quantidade: number }[]; agregada: { nome: string; qtd: number; valor: number | null }[]; totalMapeados: number; totalPendentes: number }
...
export default function VendasImportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const abrirPreview = async () => {
    if (!data) { setErro('Informe a data das vendas.'); return }
    setProcessando(true); setErro(null); setRecibo(null)
    try {
      const r = await fetch(`/api/empresas/${id}/estoque/vendas/processar`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ html, data, confirmar: false }) })
      const j = await r.json().catch(() => null)
      if (!r.ok) { setErro(j?.erro ?? 'Não consegui montar o preview.'); return }
      setPlano(j.plano)
    } catch { setErro('Falha de conexão.') } finally { setProcessando(false) }
  }
  const confirmar = async () => {
    setProcessando(true)
    try {
      const r = await fetch(`/api/empresas/${id}/estoque/vendas/processar`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ html, data, confirmar: true }) })
      const j = await r.json().catch(() => null)
      if (r.ok) { setRecibo(j.recibo); setPlano(null) } else setErro(j?.erro ?? 'Não consegui processar.')
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5 p-4 sm:p-6">
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="text-xl font-semibold text-slate-900">Vendas (Suitable)</h1><p className="text-sm text-slate-500">Sobe o relatório do dia. Conhecidos baixam sozinhos; produto novo você aponta uma vez (vale pra sempre).</p></div>
      </div>
      ...
      {recibo && (
        <Card className="border-emerald-300"><CardContent className="p-4">
          <p className="flex items-center gap-2 text-sm font-semibold text-emerald-700"><Receipt className="h-4 w-4" /> Vendas de {recibo.data.split('-').reverse().join('/')} processadas</p>
          ...
          <a href={`/empresas/${id}/estoque/movimentos?tipo=BAIXA_VENDA`} className="mt-2 inline-block text-xs text-[#185FA5] hover:underline">ver os movimentos no extrato →</a>
        </CardContent></Card>
      )}
      ...
      {preview && (
        <>
          <Card><CardContent className="grid grid-cols-3 gap-4 p-4 text-sm">
            ...
            <div><p className="text-xs text-slate-500">Pendentes de mapa</p><p className={`text-lg font-semibold ${preview.naoMapeados > 0 ? 'text-amber-600' : 'text-emerald-600'}`}>{preview.naoMapeados}</p></div>
          </CardContent></Card>

          {/* processar — SEMPRE visível, parcial é normal */}
          <Card className="border-[#185FA5]/30"><CardContent className="flex flex-wrap items-end gap-3 p-4">
            <label className="text-xs text-slate-500">Data das vendas<input type="date" value={data} onChange={(e) => setData(e.target.value)} className="mt-1 block rounded-lg border border-slate-300 py-2 px-3 text-sm" /></label>
            <button onClick={abrirPreview} disabled={processando || preview.totalProdutos - preview.naoMapeados === 0} className="inline-flex items-center gap-2 rounded-lg bg-[#185FA5] px-5 py-2.5 text-sm font-medium text-white hover:bg-[#0F4A8C] disabled:opacity-50">
              {processando ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />} Processar vendas ({preview.totalProdutos - preview.naoMapeados} mapeados · {preview.naoMapeados} pendentes)
            </button>
            <span className="text-[11px] text-slate-400">Os pendentes não baixam (mapeie depois e reprocesse o dia). Parcial é normal.</span>
          </CardContent></Card>

          {/* filtro + busca */}
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative flex-1 min-w-[180px]"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" /><input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="buscar produto…" className="w-full rounded-lg border border-slate-300 py-2 pl-9 pr-3 text-sm" /></div>
          ...
          <Card><CardContent className="p-0">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-slate-100 text-left text-xs text-slate-400"><th className="p-3 font-medium">Produto (Suitable)</th><th className="p-3 text-right font-medium">Qtd</th><th className="p-3 font-medium">Destino no estoque</th></tr></thead>
              <tbody>
      ...
      {plano && (
        <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/40 sm:items-center" onClick={() => setPlano(null)}>
          <div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-t-2xl bg-white p-5 sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between"><h3 className="text-base font-semibold text-slate-900">Confirmar baixa de {data.split('-').reverse().join('/')}</h3><button onClick={() => setPlano(null)}><X className="h-5 w-5 text-slate-400" /></button></div>
            <p className="mb-3 text-xs text-slate-500">{plano.totalMapeados} produtos vão baixar o estoque · {plano.totalPendentes} pendentes não baixam.</p>
            ...
            {plano.pendentes.length > 0 && (
              <div className="mb-3">
                <p className="mb-1 flex items-center gap-1 text-xs font-semibold text-amber-700"><Package className="h-3.5 w-3.5" /> Pendentes (não baixam — mapeie e reprocesse):</p>
                <div className="rounded-lg bg-amber-50/60 px-3 py-2 text-xs text-amber-800">{plano.pendentes.map((p) => `${p.nome} (${p.quantidade})`).join(' · ')}</div>
              </div>
            )}
