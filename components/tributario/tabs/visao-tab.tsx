// Sprint 5.0.2.c.2 — Tab Visão: cards de regime + DAS + Fator R.
// Server component. Recebe profile + lastCalc do hub.
...
import { TributarioRecalcButton } from '@/app/(dashboard)/tributario/recalc-button'
import type { CompanyTaxProfile, TaxCalculation } from '@prisma/client'

interface Props {
  empresaId: string
  profile: CompanyTaxProfile | null
  lastCalc: TaxCalculation | null
}

export function VisaoTab({ empresaId, profile, lastCalc }: Props) {
  if (!profile) {
    return (
      <Card>
        <CardContent className="py-12 text-center">
          <p className="text-sm text-zinc-600">Perfil tributário não configurado.</p>
          <Button asChild className="mt-4">
            <Link href="/tributario?tab=config">Configurar agora</Link>
          </Button>
        </CardContent>
      </Card>
    )
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-2 text-xs text-zinc-600">
                  ...
                  <Badge variant="outline">RBA {formatBRL(lastCalc.rbaAcumulada)}</Badge>
              ...
              <TributarioRecalcButton empresaId={empresaId} />
            </div>
      ...
      {!lastCalc && profile.regime === 'SIMPLES_NACIONAL' && (
        <Card className="md:col-span-2">
          <CardContent className="py-8 text-center">
            <p className="text-sm text-zinc-600 mb-3">Nenhum DAS calculado ainda.</p>
            <TributarioRecalcButton empresaId={empresaId} />
          </CardContent>
        </Card>
      )}
